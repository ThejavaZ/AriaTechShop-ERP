<div class="p-6">
    ...
    <div class="mb-6 border-b border-gray-200">
        <nav class="-mb-px flex space-x-8 overflow-x-auto">
            <button wire:click="$set('statusFilter', '')"
                ...
                Todas
                <span
                    class="ml-2 py-0.5 px-2.5 rounded-full text-xs font-medium @if ($statusFilter === '') bg-indigo-100 text-indigo-600 @else bg-gray-100 text-gray-900 @endif">
                    {{ $statusCounts['all'] }}
                </span>
            </button>
            ...
            <button wire:click="$set('statusFilter', 'pending')"
                ...
                Pendientes
                <span
                    class="ml-2 py-0.5 px-2.5 rounded-full text-xs font-medium @if ($statusFilter === 'pending') bg-yellow-100 text-yellow-600 @else bg-gray-100 text-gray-900 @endif">
            ...
            <button wire:click="$set('statusFilter', 'diagnosed')"
                ...
                Diagnosticadas
                <span
                    class="ml-2 py-0.5 px-2.5 rounded-full text-xs font-medium @if ($statusFilter === 'diagnosed') bg-blue-100 text-blue-600 @else bg-gray-100 text-gray-900 @endif">
            ...
            <button wire:click="$set('statusFilter', 'in_progress')"
                ...
                En Progreso
                <span
                    class="ml-2 py-0.5 px-2.5 rounded-full text-xs font-medium @if ($statusFilter === 'in_progress') bg-orange-100 text-orange-600 @else bg-gray-100 text-gray-900 @endif">
            ...
            <button wire:click="$set('statusFilter', 'completed')"
                ...
                Completadas
                <span
                    class="ml-2 py-0.5 px-2.5 rounded-full text-xs font-medium @if ($statusFilter === 'completed') bg-green-100 text-green-600 @else bg-gray-100 text-gray-900 @endif">
            ...
            <button wire:click="$set('statusFilter', 'delivered')"
                ...
                Entregadas
                <span
                    class="ml-2 py-0.5 px-2.5 rounded-full text-xs font-medium @if ($statusFilter === 'delivered') bg-gray-100 text-gray-600 @else bg-gray-100 text-gray-900 @endif">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Equipo
                    </th>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Técnico
                    </th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($repairs as $repair)
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-6 py-4">
                            <div class="text-sm text-gray-900 max-w-xs truncate">{{ $repair->issue_description }}
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span
    ...
    @if ($showCreateModal)
        <div class="fixed inset-0 z-50 overflow-y-auto" x-data="{ show: @entangle('showCreateModal') }">
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75" wire:click="closeCreateModal">
                </div>
                <div
                    ...
                    <form wire:submit.prevent="createRepair">
                        ...
                        <div class="bg-white px-6 py-4 space-y-4 max-h-96 overflow-y-auto">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">Tipo de Dispositivo
                                        *</label>
                                ...
                                <div>
                                    ...
                                    <input type="date" wire:model="estimated_delivery"
                                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                </div>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Descripción del Problema
                                    *</label>
    ...
    @if ($showDetailModal && $selectedRepair)
        <div class="fixed inset-0 z-50 overflow-y-auto">
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75" wire:click="closeDetailModal">
                </div>
                    ...
                    <div class="bg-white px-6 py-4 max-h-[75vh] overflow-y-auto space-y-6">
                        ...
                        <div class="bg-indigo-50 border border-indigo-200 rounded-lg p-4">
                            ...
                            @if ($showStatusChangeSuccess)
                                <div
                                    class="mb-3 flex items-center gap-2 bg-green-50 border border-green-200 text-green-800 px-3 py-2 rounded-md text-sm">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
                                ...
                                <div>
                                    <label class="block text-xs font-medium text-indigo-800 mb-1">Notas
                                        (opcional)</label>
                                    <input type="text" wire:model="statusChangeNotes"
                        ...
                        <div>
                            <dt class="text-xs text-gray-500">Técnico Asignado</dt>
                            <dd class="text-sm text-gray-900">
                                {{ $selectedRepair->technician?->name ?? 'Sin asignar' }}</dd>
                        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
        <div class="md:col-span-2">
            <label class="block text-xs font-medium text-blue-800 mb-1">Técnico</label>
            <select wire:model="newTechnicianId"
                    class="block w-full border-blue-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm">
                <option value="">Sin asignar</option>
                @foreach($technicians as $technician)
                    <option value="{{ $technician->id }}">{{ $technician->name }}</option>
                @endforeach
            </select>
            @error('newTechnicianId') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
        </div>
        <div>
            <button wire:click="updateTechnician"
                    wire:loading.attr="disabled"
                    class="w-full inline-flex justify-center items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white rounded-md text-sm font-medium transition">
                <span wire:loading.remove wire:target="updateTechnician">Asignar</span>
                <span wire:loading wire:target="updateTechnician">Guardando...</span>
            </button>
        </div>
    </div>
</div>
                            ...
                            <div>
                                ...
                                <dl class="space-y-1">
                                    ...
                                    <div>
                                        ...
                                        <dd class="text-sm text-gray-900">{{ $selectedRepair->brand }}
                                            {{ $selectedRepair->model }}</dd>
                                    </div>
                                    @if ($selectedRepair->serial_number)
                                        <div>
                                            <dt class="text-xs text-gray-500">Serie</dt>
                                            <dd class="text-sm text-gray-900">{{ $selectedRepair->serial_number }}
                                            </dd>
                                        </div>
                                    @endif
                                </dl>
                            ...
                            <div>
                                ...
                                <dl class="space-y-1">
                                    ...
                                    <div>
                                        ...
                                        <dd class="text-sm text-gray-900">
                                            {{ $selectedRepair->received_at->format('d/m/Y') }}</dd>
                                    </div>
                                    @if ($selectedRepair->estimated_delivery)
                                        <div>
                                            <dt class="text-xs text-gray-500">Entrega Estimada</dt>
                                            <dd class="text-sm text-gray-900">
                                                {{ $selectedRepair->estimated_delivery->format('d/m/Y') }}</dd>
                                        </div>
                                    @endif
                                </dl>
                            </div>
                                ...
                                <dl class="space-y-1">
                                    @if ($selectedRepair->estimated_cost)
                                        <div>
                                            ...
                                            <dd class="text-sm text-gray-900">
                                                ${{ number_format($selectedRepair->estimated_cost, 2) }}</dd>
                                        </div>
                                    @endif
                                    @if ($selectedRepair->final_cost)
                                        <div>
                                            ...
                                            <dd class="text-sm font-medium text-gray-900">
                                                ${{ number_format($selectedRepair->final_cost, 2) }}</dd>
                                        </div>
                                    @endif
                                </dl>
                            </div>
                                                            ...
                                                            <div class="flex-1 min-w-0">
                                                                <div>
                                                                    <p class="text-sm text-gray-500">
                                                                        Cambió a <span
                                                                            class="font-medium text-gray-900">{{ ucfirst(str_replace('_', ' ', $history->status_to)) }}</span>
                                                                    </p>
    ...
    @if ($showDeleteModal && $repairToDelete)
        <div class="fixed inset-0 z-50 overflow-y-auto">
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75" wire:click="closeDeleteModal">
                </div>
                <div
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">
                                        ¿Estás seguro de eliminar la reparación
                                        <strong>{{ $repairToDelete->repair_number }}</strong>?
